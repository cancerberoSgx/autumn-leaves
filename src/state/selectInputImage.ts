...
export const SELECT_INPUT_IMAGE: string = 'select-input-image'
export interface SelectInputImageAction extends Action {

}

export function selectInputImage(state: State, action: SelectInputImageAction): State {
  // state = State.get()
  // if (action.type !== ACTION_ADD_UNIT) {
  //   return state
  // }
  return selectInputImageImpl(state, action)
}


export function selectInputImageImpl(state: State, action: SelectInputImageAction): State {
  return null
  // state = State.get()

...
class SwirlMorph implements Morph {
  name = "swirl"
  id = "swirl"
  ...
  arguments: Argument[] = [
...

    {
      ...
      description: "If true it will perform the transformation twice",
      defaultValue: true
    } as Argument,
    {
      type: ArgumentType.number,
  ...
  async template(config: MorphConfig & { arguments: Arguments & { frames: number, swirlFactor: number } }) {
    ...
    const fileNames1 = []
    const fileNames2 = []
    console.log(config.arguments.backAndForwards)
    ...
    const commands = `
      ...
      convert -morph 2 -delay 100 ${inputFiles[0].name} -delay 10 ${[].concat(fileNames1).concat(fileNames2.reverse()).join(" ")} -delay 200 ${inputFiles[1].name} ${config.arguments.backAndForwards ? `-delay 10 ${fileNames2.reverse().join(" ")} ${fileNames1.reverse().join(" ")} ${inputFiles[0].name}` : ""} -loop 0 -layers Optimize out${getUniqueId()}.gif`
...
class ComposeMorph implements Morph {
  ...
  async template(config) {
    ...
    convert -delay 50 f0.miff -morph 4 -delay 20 ${list.map(i => `compose_output_${i}.miff`).join(" ")} -delay 200 f1.miff -delay 20 ${list.map(i => `compose_output_${i}.miff`).reverse().join(" ")} -layers Optimize output.gif
    `
    const result = await execute({ inputFiles, commands })
    return [result.outputFiles[result.outputFiles.length - 1]]
  }
...
class PixelatedMorph implements Morph {
  ...
  async template(config) {
    ...
    convert -delay 50 f0.miff -morph 2 -delay 20 ${list.map((n, i) => `f0_${i}.miff`).join(" ")} ${list.map((n, i) => `f1_${i}.miff`).reverse().join(" ")} -delay 200 f1.miff -layers Optimize output.gif
    `
...
export const morphs: Morph[] = [
...

  {
   ...
   convert  -delay 100 $$IMAGES \\
      ...
      -loop 0 -layers Optimize wipe_all.gif

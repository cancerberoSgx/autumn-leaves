...
export class TextBannerBlurShadowFont implements MagickTemplate {
  name = "Text Banner Blur Shadow"
  id = "textBannerBlurShadowFont"
  description = `TODO`
...
convert -size ${w}x${h} xc:${config.arguments.backgroundColor} -gravity center -font '${fontName}' -pointsize ${config.arguments.fontSize} \\
  -fill ${config.arguments.shadowColor} -annotate 0 '${config.arguments.text}' -motion-blur 0x25+65 \\
  -fill ${config.arguments.textColor}  -annotate 0 '${config.arguments.text}' -motion-blur 0x1+65 \\
